...
struct GroupDetailView: View {
    ...
    private func sendMessage() async throws {
        ...
        if let selectedImage = groupDetailConfig.selectedImage {
            guard let resizedImagec = selectedImage.resize(to: CGSize(width: 600, height: 600)),
                  let imageData = resizedImagec.pngData() else { return }
    ...
    var body: some View {
        ...
        }).sheet(item: $groupDetailConfig.sourceType, content: { source in
            ImagePicker(image: $groupDetailConfig.selectedImage,sourceType: source)
        })
        .padding()
